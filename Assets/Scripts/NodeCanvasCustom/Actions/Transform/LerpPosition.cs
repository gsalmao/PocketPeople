...
namespace NodeCanvasCustom.Actions
{
	...
	public class LerpPosition : ActionTask<Transform>
	{
		...
		protected override void OnUpdate()
		{
			...
			currentPosition.x = Mathf.LerpUnclamped(initPosition.x, target.value.position.x, curveX.value.Evaluate(t));
			currentPosition.y = Mathf.LerpUnclamped(initPosition.y, target.value.position.y, curveY.value.Evaluate(t));
